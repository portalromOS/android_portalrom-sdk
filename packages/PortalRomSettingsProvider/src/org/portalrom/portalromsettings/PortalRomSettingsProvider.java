...
/**
 * The PortalRomSettingsProvider serves as a {@link ContentProvider} for PortalRom specific settings
 */
public class PortalRomSettingsProvider extends ContentProvider {
    public static final String TAG = "PortalRomSettingsProvider";
    ...
    public static final String PREF_HAS_MIGRATED_PORTALROM_SETTINGS =
            "migrated_settings_to_portalrom_17_0";

    private static final Bundle NULL_SETTING = Bundle.forPair("value", null);

    // Each defined user has their own settings
    protected final SparseArray<PortalRomDatabaseHelper> mDbHelpers = new SparseArray<PortalRomDatabaseHelper>();
    ...
    static {
        sUriMatcher.addURI(PortalRomSettings.AUTHORITY, PortalRomDatabaseHelper.PortalRomTableNames.TABLE_SYSTEM,
                SYSTEM);
        sUriMatcher.addURI(PortalRomSettings.AUTHORITY, PortalRomDatabaseHelper.PortalRomTableNames.TABLE_SECURE,
                SECURE);
        sUriMatcher.addURI(PortalRomSettings.AUTHORITY, PortalRomDatabaseHelper.PortalRomTableNames.TABLE_GLOBAL,
                GLOBAL);
        sUriMatcher.addURI(PortalRomSettings.AUTHORITY, PortalRomDatabaseHelper.PortalRomTableNames.TABLE_SYSTEM +
    ...
    public boolean onCreate() {
        if (LOCAL_LOGV) Log.d(TAG, "Creating PortalRomSettingsProvider");
        ...
        mUriBuilder.authority(PortalRomSettings.AUTHORITY);
    ...
    /**
     * Migrates PortalRom settings for all existing users if this has not been run before.
     */
    private void migratePortalRomSettingsForExistingUsersIfNeeded() {
        boolean hasMigratedPortalRomSettings = mSharedPrefs.getBoolean(PREF_HAS_MIGRATED_PORTALROM_SETTINGS,
                false);

        if (!hasMigratedPortalRomSettings) {
            ...
            for (UserInfo user : mUserManager.getUsers()) {
                migratePortalRomSettingsForUser(user.id);
            }

            mSharedPrefs.edit().putBoolean(PREF_HAS_MIGRATED_PORTALROM_SETTINGS, true).commit();
    ...
    /**
     * Migrates PortalRom settings for a specific user.
     * @param userId The id of the user to run PortalRom settings migration for.
     */
    private void migratePortalRomSettingsForUser(int userId) {
        synchronized (this) {
            if (LOCAL_LOGV) Log.d(TAG, "PortalRom settings will be migrated for user id: " + userId);

            // Rename database files (if needed)
            PortalRomDatabaseHelper dbHelper = mDbHelpers.get(userId);
            if (dbHelper != null) {
                dbHelper.close();
                mDbHelpers.delete(userId);
            }
            PortalRomDatabaseHelper.migrateDbFiles(getContext(), userId);
            if (dbHelper != null) {
            ...
            int rowsMigrated = migratePortalRomSettingsForTable(userId,
                    PortalRomDatabaseHelper.PortalRomTableNames.TABLE_SYSTEM, PortalRomSettings.System.LEGACY_SYSTEM_SETTINGS);
            if (LOCAL_LOGV) Log.d(TAG, "Migrated " + rowsMigrated + " to PortalRom system table");

            // Migrate secure settings
            rowsMigrated = migratePortalRomSettingsForTable(userId,
                    PortalRomDatabaseHelper.PortalRomTableNames.TABLE_SECURE, PortalRomSettings.Secure.LEGACY_SECURE_SETTINGS);
            if (LOCAL_LOGV) Log.d(TAG, "Migrated " + rowsMigrated + " to PortalRom secure table");

            // Migrate global settings
            rowsMigrated = migratePortalRomSettingsForTable(userId,
                    PortalRomDatabaseHelper.PortalRomTableNames.TABLE_GLOBAL, PortalRomSettings.Global.LEGACY_GLOBAL_SETTINGS);
            if (LOCAL_LOGV) Log.d(TAG, "Migrated " + rowsMigrated + " to PortalRom global table");
        }
    }

    /**
     * Migrates PortalRom settings for a specific table and user id.
     * @param userId The id of the user to run PortalRom settings migration for.
     * @param tableName The name of the table to run PortalRom settings migration on.
     * @param settings An array of keys to migrate from {@link Settings} to {@link PortalRomSettings}
     * @return Number of rows migrated.
     */
    private int migratePortalRomSettingsForTable(int userId, String tableName, String[] settings) {
        ...
        for (String settingsKey : settings) {
            ...
            if (tableName.equals(PortalRomDatabaseHelper.PortalRomTableNames.TABLE_SYSTEM)) {
                settingsValue = Settings.System.getStringForUser(contentResolver, settingsKey,
                        userId);
            }
            else if (tableName.equals(PortalRomDatabaseHelper.PortalRomTableNames.TABLE_SECURE)) {
                settingsValue = Settings.Secure.getStringForUser(contentResolver, settingsKey,
                        userId);
                if (settingsValue != null && settingsKey.equals(PortalRomSettings.Secure.STATS_COLLECTION)
                        && PortalRomSettings.Secure.getStringForUser(contentResolver, settingsKey, userId)
                        != null) {
            ...
            else if (tableName.equals(PortalRomDatabaseHelper.PortalRomTableNames.TABLE_GLOBAL)) {
    ...
    public Bundle call(String method, String request, Bundle args) {
        ...
        if (args != null) {
            int reqUser = args.getInt(PortalRomSettings.CALL_METHOD_USER_KEY, callingUserId);
        ...
        switch (method) {
            // Migrate methods
           case PortalRomSettings.CALL_METHOD_MIGRATE_SETTINGS:
                migratePortalRomSettingsForExistingUsersIfNeeded();
                return null;
           case PortalRomSettings.CALL_METHOD_MIGRATE_SETTINGS_FOR_USER:
                migratePortalRomSettingsForUser(callingUserId);
                return null;

            // Get methods
            case PortalRomSettings.CALL_METHOD_GET_SYSTEM:
                return lookupSingleValue(callingUserId, PortalRomSettings.System.CONTENT_URI,
                        request);
            case PortalRomSettings.CALL_METHOD_GET_SECURE:
                return lookupSingleValue(callingUserId, PortalRomSettings.Secure.CONTENT_URI,
                        request);
            case PortalRomSettings.CALL_METHOD_GET_GLOBAL:
                return lookupSingleValue(callingUserId, PortalRomSettings.Global.CONTENT_URI,
                        request);

            // Put methods
            case PortalRomSettings.CALL_METHOD_PUT_SYSTEM:
                enforceWritePermission(portalrom.platform.Manifest.permission.WRITE_SETTINGS);
                callHelperPut(callingUserId, PortalRomSettings.System.CONTENT_URI, request, args);
                return null;
            case PortalRomSettings.CALL_METHOD_PUT_SECURE:
                enforceWritePermission(
                        portalrom.platform.Manifest.permission.WRITE_SECURE_SETTINGS);
                callHelperPut(callingUserId, PortalRomSettings.Secure.CONTENT_URI, request, args);
                return null;
            case PortalRomSettings.CALL_METHOD_PUT_GLOBAL:
                enforceWritePermission(
                        portalrom.platform.Manifest.permission.WRITE_SECURE_SETTINGS);
                callHelperPut(callingUserId, PortalRomSettings.Global.CONTENT_URI, request, args);
                return null;

            // List methods
            case PortalRomSettings.CALL_METHOD_LIST_SYSTEM:
                return callHelperList(callingUserId, PortalRomSettings.System.CONTENT_URI);
            case PortalRomSettings.CALL_METHOD_LIST_SECURE:
                return callHelperList(callingUserId, PortalRomSettings.Secure.CONTENT_URI);
            case PortalRomSettings.CALL_METHOD_LIST_GLOBAL:
                return callHelperList(callingUserId, PortalRomSettings.Global.CONTENT_URI);

            // Delete methods
            case PortalRomSettings.CALL_METHOD_DELETE_SYSTEM:
                enforceWritePermission(portalrom.platform.Manifest.permission.WRITE_SETTINGS);
                return callHelperDelete(callingUserId, PortalRomSettings.System.CONTENT_URI,
                        request);
            case PortalRomSettings.CALL_METHOD_DELETE_SECURE:
                enforceWritePermission(
                        portalrom.platform.Manifest.permission.WRITE_SECURE_SETTINGS);
                return callHelperDelete(callingUserId, PortalRomSettings.Secure.CONTENT_URI,
                        request);
            case PortalRomSettings.CALL_METHOD_DELETE_GLOBAL:
                enforceWritePermission(
                        portalrom.platform.Manifest.permission.WRITE_SECURE_SETTINGS);
                return callHelperDelete(callingUserId, PortalRomSettings.Global.CONTENT_URI,
                        request);
        }
    ...
    private Cursor queryForUser(int userId, Uri uri, String[] projection, String selection,
        ...
        PortalRomDatabaseHelper dbHelper = getOrEstablishDatabase(getUserIdForTable(tableName, userId));
    ...
    private Uri insertForUser(int userId, Uri uri, ContentValues values) {
        ...
        if (PortalRomDatabaseHelper.PortalRomTableNames.TABLE_GLOBAL.equals(tableName)) {
            validateGlobalSettingNameValue(name, value);
        } else if (PortalRomDatabaseHelper.PortalRomTableNames.TABLE_SYSTEM.equals(tableName)) {
            validateSystemSettingNameValue(name, value);
        } else if (PortalRomDatabaseHelper.PortalRomTableNames.TABLE_SECURE.equals(tableName)) {
    ...
    private int deleteForUser(int callingUserId, Uri uri, String selection,
        ...
        if (!TextUtils.isEmpty(selection) && selectionArgs.length > 0) {
            ...
            PortalRomDatabaseHelper dbHelper = getOrEstablishDatabase(getUserIdForTable(tableName,
                    callingUserId));
    ...
    public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        // NOTE: update() is never called by the front-end PortalRomSettings API, and updates that
    ...
    /**
     * Tries to get a {@link PortalRomDatabaseHelper} for the specified user and if it does not exist, a
     * new instance of {@link PortalRomDatabaseHelper} is created for the specified user and returned.
     * @param callingUser
     * @return
     */
    private PortalRomDatabaseHelper getOrEstablishDatabase(int callingUser) {
        ...
        try {
            PortalRomDatabaseHelper dbHelper;
            synchronized (this) {
    ...
    /**
     * Check if a {@link PortalRomDatabaseHelper} exists for a user and if it doesn't, a new helper is
    ...
    private void establishDbTracking(int userId) {
        ...
        synchronized (this) {
            ...
            if (LOCAL_LOGV) {
                Log.i(TAG, "Checking portalrom settings db helper for user " + userId);
            }
            if (dbHelper == null) {
                if (LOCAL_LOGV) {
                    Log.i(TAG, "Installing new portalrom settings db helper for user " + userId);
                }
                dbHelper = new PortalRomDatabaseHelper(getContext(), userId);
    ...
    private void checkWritePermissions(String tableName) {
        ...
        final boolean protectedTable =
                PortalRomDatabaseHelper.PortalRomTableNames.TABLE_SECURE.equals(tableName) ||
                PortalRomDatabaseHelper.PortalRomTableNames.TABLE_GLOBAL.equals(tableName);
        ...
        throw new SecurityException(
                String.format("Permission denial: writing to portalrom settings requires %1$s",
                        portalrom.platform.Manifest.permission.WRITE_SECURE_SETTINGS));
    }
    ...
    private String getTableNameFromUriMatchCode(int code) {
        switch (code) {
            case SYSTEM:
            case SYSTEM_ITEM_NAME:
                return PortalRomDatabaseHelper.PortalRomTableNames.TABLE_SYSTEM;
            case SECURE:
            case SECURE_ITEM_NAME:
                return PortalRomDatabaseHelper.PortalRomTableNames.TABLE_SECURE;
            case GLOBAL:
            case GLOBAL_ITEM_NAME:
                return PortalRomDatabaseHelper.PortalRomTableNames.TABLE_GLOBAL;
            default:
                throw new IllegalArgumentException("Invalid uri match code: " + code);
        }
    }
    ...
    private int getUserIdForTable(String tableName, int userId) {
        return PortalRomDatabaseHelper.PortalRomTableNames.TABLE_GLOBAL.equals(tableName) ?
                UserHandle.USER_SYSTEM : userId;
    }

    /**
     * Modify setting version for an updated table before notifying of change. The
     * {@link PortalRomSettings} class uses these to provide client-side caches.
     * @param uri to send notifications for
     * @param userId
     */
    private void notifyChange(Uri uri, String tableName, int userId) {
        String property = null;
        final boolean isGlobal = tableName.equals(PortalRomDatabaseHelper.PortalRomTableNames.TABLE_GLOBAL);
        if (tableName.equals(PortalRomDatabaseHelper.PortalRomTableNames.TABLE_SYSTEM)) {
            property = PortalRomSettings.System.SYS_PROP_PORTALROM_SETTING_VERSION;
        } else if (tableName.equals(PortalRomDatabaseHelper.PortalRomTableNames.TABLE_SECURE)) {
            property = PortalRomSettings.Secure.SYS_PROP_PORTALROM_SETTING_VERSION;
        } else if (isGlobal) {
            property = PortalRomSettings.Global.SYS_PROP_PORTALROM_SETTING_VERSION;
        }
    ...
    private void validateGlobalSettingNameValue(String name, String value) {
        PortalRomSettings.Validator validator = PortalRomSettings.Global.VALIDATORS.get(name);
    ...
    private void validateSystemSettingNameValue(String name, String value) {
        PortalRomSettings.Validator validator = PortalRomSettings.System.VALIDATORS.get(name);
        if (validator == null) {
    ...
    private void validateSecureSettingValue(String name, String value) {
        PortalRomSettings.Validator validator = PortalRomSettings.Secure.VALIDATORS.get(name);
