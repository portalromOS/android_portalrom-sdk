...
import java.util.Map;
 ...
 public class PortalRomSettingsProviderTest extends AndroidTestCase {
     private static final String TAG = "PortalRomSettingsProviderTest";
     ...
     @MediumTest
     public void testMigratePortalRomSettingsForOtherUser() {
     ...
     private void testMigrateSettingsForUser(int userId) {
         ...
         Settings.System.putStringForUser(mContentResolver,
                 PortalRomSettingsProvider.LegacyPortalRomSettings.STATUS_BAR_QUICK_QS_PULLDOWN,
                 expectedPullDownValue, userId);

         final int expectedKeyboardBrightness = 4;
         Settings.Secure.putIntForUser(mContentResolver,
                 PortalRomSettingsProvider.LegacyPortalRomSettings.KEYBOARD_BRIGHTNESS,
                 expectedKeyboardBrightness, userId);

         Bundle arg = new Bundle();
         arg.putInt(PortalRomSettings.CALL_METHOD_USER_KEY, userId);
         ...
         try{
             ...
             contentProvider.call(mContentResolver.getPackageName(),
                     PortalRomSettings.CALL_METHOD_MIGRATE_SETTINGS_FOR_USER, null, arg);
         } catch (RemoteException ex) {
             fail("Failed to trigger settings migration due to RemoteException");
         }

         // Check values
         final String actualPullDownValue = PortalRomSettings.System.getStringForUser(mContentResolver,
                 PortalRomSettings.System.QS_QUICK_PULLDOWN, userId);
         assertEquals(expectedPullDownValue, actualPullDownValue);

         final int actualKeyboardBrightness = PortalRomSettings.Secure.getIntForUser(mContentResolver,
                 PortalRomSettings.Secure.KEYBOARD_BRIGHTNESS, -1, userId);
     ...
     public void testBulkInsertSuccess() {
         ...
         testBulkInsertForUri(PortalRomSettings.System.CONTENT_URI, contentValues, keyValues);
         testBulkInsertForUri(PortalRomSettings.Secure.CONTENT_URI, contentValues, keyValues);
         testBulkInsertForUri(PortalRomSettings.Global.CONTENT_URI, contentValues, keyValues);
     }
     ...
     public void testInsertUpdateDeleteSuccess() {
         //testInsertUpdateDeleteForUri(PortalRomSettings.System.CONTENT_URI);
         testInsertUpdateDeleteForUri(PortalRomSettings.Secure.CONTENT_URI);
         testInsertUpdateDeleteForUri(PortalRomSettings.Global.CONTENT_URI);
     }
