...
import static portalrom.hardware.LiveDisplayManager.MODE_OUTDOOR;
...
public class OutdoorModeController extends LiveDisplayFeature {
    ...
    public OutdoorModeController(Context context, Handler handler) {
        ...
        mUseOutdoorMode = mHardware.isSupported(PortalRomHardwareManager.FEATURE_SUNLIGHT_ENHANCEMENT);

        mDefaultOutdoorLux = mContext.getResources().getInteger(
                org.portalrom.platform.internal.R.integer.config_outdoorAmbientLux);
        mOutdoorLuxHysteresis = mContext.getResources().getInteger(
                org.portalrom.platform.internal.R.integer.config_outdoorAmbientLuxHysteresis);
        mDefaultAutoOutdoorMode = mContext.getResources().getBoolean(
                org.portalrom.platform.internal.R.bool.config_defaultAutoOutdoorMode);
    }
    ...
    public void onStart() {
        ...
        registerSettings(
                PortalRomSettings.System.getUriFor(PortalRomSettings.System.DISPLAY_AUTO_OUTDOOR_MODE));
    }
    ...
    protected synchronized void onScreenStateChanged() {
        ...
        if (!isScreenOn() && getMode() != MODE_OUTDOOR) {
            mIsOutdoor = false;
            mHardware.set(PortalRomHardwareManager.FEATURE_SUNLIGHT_ENHANCEMENT, false);
        }
    }
    ...
    public void dump(PrintWriter pw) {
        ...
        pw.println("    hardware state=" +
                mHardware.get(PortalRomHardwareManager.FEATURE_SUNLIGHT_ENHANCEMENT));
    ...
    private synchronized void updateOutdoorMode() {
        ...
        if (isScreenOn()) {
            ...
            mHardware.set(PortalRomHardwareManager.FEATURE_SUNLIGHT_ENHANCEMENT, enabled);
        }
    }
    ...
    boolean setAutomaticOutdoorModeEnabled(boolean enabled) {
        ...
        putBoolean(PortalRomSettings.System.DISPLAY_AUTO_OUTDOOR_MODE, enabled);
        return true;
    }

    boolean isAutomaticOutdoorModeEnabled() {
        return mUseOutdoorMode && (mNightDisplayAvailable ||
                getBoolean(PortalRomSettings.System.DISPLAY_AUTO_OUTDOOR_MODE,
