...
import org.portalrom.platform.internal.common.PortalRomSystemServiceHelper;

/**
 * Base PortalRom System Server which handles the starting and states of various PortalRom
 ...
 */
public class PortalRomSystemServer {
    private static final String TAG = "PortalRomSystemServer";
    private Context mSystemContext;
    private PortalRomSystemServiceHelper mSystemServiceHelper;

    private static final String ENCRYPTING_STATE = "trigger_restart_min_framework";
    private static final String ENCRYPTED_STATE = "1";

    public PortalRomSystemServer(Context systemContext) {
        mSystemContext = systemContext;
        mSystemServiceHelper = new PortalRomSystemServiceHelper(mSystemContext);
    }
    ...
    private void run() {
        ...
        try {
            ...
            Slog.e("System", "******************************************");
            Slog.e("System", "************ Failure starting portalrom system services", ex);
            throw ex;
        }
    }
    ...
    private void startServices() {
        ...
        String[] externalServices = context.getResources().getStringArray(
                org.portalrom.platform.internal.R.array.config_externalPortalRomServices);

        for (String service : externalServices) {
            try {
                Slog.i(TAG, "Attempting to start service " + service);
                PortalRomSystemService portalromSystemService =  mSystemServiceHelper.getServiceFor(service);
                if (context.getPackageManager().hasSystemFeature(
                        portalromSystemService.getFeatureDeclaration())) {
                    if (coreAppsOnly() && !portalromSystemService.isCoreService()) {
                        Slog.d(TAG, "Not starting " + service +
                                " - only parsing core apps");
                    } else {
                        Slog.i(TAG, "Starting service " + service);
                        ssm.startService(portalromSystemService.getClass());
                    }
                } else {
